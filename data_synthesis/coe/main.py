...
import json
import numpy as np
from coe.comment import Comment
from coe.conductor import Conductor
from coe.reducer import Reducer
from coe.evaluator import Evaluator
from coe.experts import (
    ModelingExpert, 
    ProgrammingExpert,
    LPFileGenerator,
...
from coe.comment_pool import CommentPool
from coe.utils import extract_code_from_string
...
def chain_of_experts(problem, 
    ...
    for _ in range(max_trials):
        ...
        #     f.write(f'<think>{comment_pool.get_current_comment_text()}</think>')
        #     f.write(f'{code}')
    ...
    return f'<think>{comment_pool.get_current_comment_text()}</think>{code}'


if __name__ == '__main__':
    from utils import read_problem
    problem = read_problem('LPWP', 'prob_250')
    chain_of_experts(problem, max_collaborate_nums=5, model_name=get_default_model(), enable_reflection=False, max_trials=1)
